...
const Home = () => {
    ...
    const fetchContent = async () => {
      try {
        const response = await fetch(
          "http://localhost:5000/api/content/public"
        );
        if (!response.ok)
  ...
  const latestNewsArticles = content?.latestNewsArticles ||
    content?.latestNews || [
      {
      ...
      },
      {
        ...
        image:
          "https://swarajya.gumlet.io/swarajya/2024-04/40a53b4d-7dc8-4017-8d35-fd3fdd3e08f6/10_04_3.png?w=1200&h=675&auto=format%2Ccompress&fit=max&enlarge=true",
      ...
      },
      {
        ...
        image:
          "https://thedailyguardian.com/wp-content/uploads/2025/01/political-landscape-in-Bihar.webp",
  ...
  const videoNewsTitle =
    content?.videoNewsTitle ||
    content?.videoNews?.title ||
    "बिहार की ताज़ा राजनीतिक खबरें | नीतीश कुमार का बड़ा बयान";
  ...
  const marqueeItems =
    content?.marqueeItems && content.marqueeItems.length > 0
            ...
            <div className="w-full lg:w-1/2">
              <div className="bg-white rounded-xl shadow-xl overflow-hidden h-full transform transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 border-2 border-gray-200">
                ...
                <div className="p-4 bg-gradient-to-b from-gray-50 to-white text-center">
                  ...
                  <div className=" h-[200px] w-full bg-gradient-to-r from-white via-red-50 to-red-100 p-6 rounded-2xl shadow-lg flex flex-col sm:flex-row items-center justify-between mt-1">
                    {/* Text Content */}
                    <div className="flex-1">
                      <h2 className="text-2xl sm:text-3xl font-extrabold text-red-600 mb-2">
                        Harshit Ke Kalam Se
                      </h2>
                    </div>
                    {/* Call to Action */}
                    <div className="mt-4 sm:mt-0">
                      <button className="bg-red-600 hover:bg-red-700 text-white font-semibold px-6 py-2 rounded-full transition shadow-md">
                        अभी विज्ञापन शुरू करें
                      </button>
                    </div>
                  </div>
                </div>
      ...
      {latestNewsArticles.length === 0 && (
        <div className="container mx-auto px-4 py-8">
          <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded">
            <strong>Debug Info:</strong> No latest news articles found. Backend
            data:{" "}
            {JSON.stringify(
              content?.latestNewsArticles || content?.latestNews || "No data"
            )}
          </div>
        </div>
      )}
            ...
            <div key={cat} className="mb-16">
              <div className="flex items-center gap-3 mb-6">
                <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-0 text-blue-700 drop-shadow-lg">
                  {cat} <span className="font-light text-gray-700">समाचार</span>
                </h2>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {content?.newsPosts?.filter((post) => post.category === cat)
                  .length > 0 ? (
                  content.newsPosts
